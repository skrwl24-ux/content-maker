...
주제 선정은 개별 아파트 단지를 중심으로 최근 3~6개월의 가격 변화·상승액·거래량 변화·저점 대비 반등 등 확인 가능한 흐름을 우선하세요. 자료에 따라 기간을 조정하고, 자치구별 주간 가격지수 순위는 사용자가 요청한 경우에만 선택하세요. 사용자가 지정한 주제와 기간이 있다면 우선하세요.

제목은 분석 기간보다 자료에서 확인된 변화와 독자의 궁금증을 앞세우세요. '최근 6개월', '최근', '요즘', 'TOP3'를 제목마다 기계적으로 붙이지 마세요. 거래 증가, 가격대 전환, 반등, 같은 지역 내 흐름 차이 등 해당 자료에 맞는 관점과 문장 구조를 선택하세요. '거래 몰렸다', '반등했다', '다시 ○억대' 같은 표현은 실제 비교 자료로 확인될 때만 쓰고, 근거 없는 급등·폭등·매수 유도 표현은 사용하지 마세요. 제목 후보를 내부적으로 비교해 가장 적합한 최종 제목 하나만 [제목]에 넣으세요. 정확한 분석 시작일·종료일과 자료 기준일은 본문에 명시하고, 그래프와 수치 비교 이미지에도 표시하세요. 썸네일은 최종 제목의 핵심 변화를 짧게 표현하되 새로운 주장을 추가하지 마세요.
