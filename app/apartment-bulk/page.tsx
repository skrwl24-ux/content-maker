...
- PC와 모바일 모두 읽기 좋게 문단 사이 여백이 느껴지도록 짧게 끊어 쓸 것
- 소제목은 짧고 명확하게, 본문은 긴 문장보다 짧은 문장을 우선할 것
- 네이버 블로그 편집 기준 글자 크기: 제목 20pt / 소제목 18pt / 본문 15pt / 태그 13~14pt
- 제목은 가장 크게, 소제목은 본문보다 확실히 크게 보이도록 구조를 구분할 것
- 소제목은 굵게 보이도록 작성하고, 본문은 기본 굵기로 유지할 것
- PC와 모바일에서 모두 읽기 좋도록 본문 한 줄이 지나치게 길어지지 않게 문장을 짧게 구성할 것
...
[띄어쓰기·가독성]
- 제목 다음에는 한 줄 비우고 본문을 시작
- 소제목 앞뒤에는 한 줄씩 비우기
- 모든 문장은 마침표·물음표·느낌표로 끝날 때마다 엔터 1회로 줄바꿈할 것
- 같은 문단 안에서도 문장마다 한 줄씩 따로 표시할 것
- 문단과 문단 사이는 엔터 2회로 빈 줄 1개를 만들 것
- 줄 시작에 불필요한 스페이스바 공백은 넣지 말 것
- 한 문단은 1~3문장으로 유지
- 숫자와 단위는 붙여 쓰되, 문장 안의 단어 띄어쓰기는 자연스러운 표준 한국어로 작성
- 모바일에서 한 덩어리로 보이지 않도록 긴 문단을 만들지 말 것
