...
- 제목은 가장 크게, 소제목은 본문보다 크게, 일반 본문은 기본 크기로 보이게 할 것.
- 모든 일반 본문 문장은 '한 문장 = 한 문단'으로 작성할 것.
- 모든 문장과 다음 문장 사이에는 반드시 한 줄을 띄울 것.
- 제목 다음에도 한 줄 띄울 것.
- 소제목 앞뒤에도 각각 한 줄 띄울 것.
- 이미지 위치 문구 앞뒤에도 각각 한 줄 띄울 것.
- 3줄 요약의 각 문장 사이에도 한 줄 띄울 것.
- 한 줄 띄기용 줄에는 네이버 붙여넣기 시 간격이 유지되도록 줄바꿈 보존용 NBSP(U+00A0) 1칸만 넣을 것.
- 즉 '문장 → 엔터 → NBSP 1칸만 있는 줄 → 엔터 → 다음 문장' 형식을 전체 글에 반복할 것.
- 완전히 비어 있는 줄은 만들지 말 것.
- 일반 텍스트가 있는 줄의 시작에는 불필요한 공백을 넣지 말 것.
- 코드블록과 HTML은 사용하지 말 것.
- 최종 출력 직전에 모든 문장 사이가 실제로 한 줄씩 띄워져 있는지 다시 확인할 것.
- 일반 ASCII 스페이스만 있는 줄은 채팅 화면에서 사라질 수 있으므로 사용하지 말 것.
- 한 줄 띄기용 줄에는 렌더링 후에도 유지되는 NBSP(U+00A0) 1칸만 넣을 것.
- 화면에서 문장과 문장 사이에 실제 빈 한 줄이 보이는 것을 최우선으로 할 것.
