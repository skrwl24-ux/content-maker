...
주의: 위 월 대표값은 '최근 실거래가' 한 건과 다른 개념이므로 제목과 본문에서 서로 섞지 말 것.

[최신 정보 확인]
웹 검색이 가능하면 본문 작성 전에 이 단지와 직접 관련 있는 최신 정보만 확인해줘.
교통, 정비사업, 공급, 생활권, 개발계획 중 실제 관련 있는 내용만 사용하고,
공식자료를 우선하며 계획·추진·착공·개통 목표·확정을 구분해줘.
확인되지 않은 호재는 쓰지 말고, 웹 검색이 불가능하면 최신 정보를 추정해서 쓰지 마.
사이트에서 계산한 역 거리는 직선거리이므로 도보시간으로 임의 환산하지 마.

[작성 규칙]
- 제목 후보를 보여주지 말고 최종 제목 1개만 작성
...
- 문장 → 엔터 → 스페이스바 1칸 → 엔터 → 다음 문장 형식으로 작성할 것
- PC와 모바일 모두 답답하지 않도록 짧은 문장을 우선하고 긴 문단을 만들지 말 것
- 소제목은 짧고 명확하게 작성할 것
- 일반 본문은 네이버 블로그의 기본 본문 크기를 그대로 사용할 것
- 제목과 소제목만 본문보다 크게 적용할 수 있도록 각각 독립된 줄로 명확하게 구분할 것
- 제목은 가장 크게, 소제목은 본문보다 한 단계 크게 보이도록 구성하되 pt 숫자 지시문은 출력하지 말 것
- 소제목은 굵게, 일반 본문은 기본 굵기로 유지할 것
- 본문 안의 핵심 숫자만 필요한 경우 굵게 강조하고 문장 전체를 과하게 굵게 처리하지 말 것
- PC와 모바일에서 모두 읽기 좋도록 한 문장이 지나치게 길어지지 않게 구성할 것
- 과장, 매수 권유, 투자 확정 표현 금지
- 제공되지 않은 실거래 숫자를 임의로 만들지 말 것
- 전용면적은 '전용 84㎡대' 같은 방식으로 표현
- 이모지는 🏠📊🚉🔎✅📌 정도만 자연스럽게 사용
- 단지명, 지역명, 아파트 실거래가, 아파트 시세 키워드를 자연스럽게 포함
- 최신 외부 정보를 쓸 때는 본문 안에서 출처 기관이나 자료 성격이 자연스럽게 드러나게 작성
- 본문 전체에서 개별 계약 가격은 반드시 '최근 실거래가' 또는 '개별 실거래가'로 표현할 것
- 월별 통계값은 반드시 '월 대표값'으로 표현하고 '최근 실거래가'와 같은 개념처럼 섞지 말 것
- 원인 설명은 확인된 사실과 해석을 구분할 것
- 공식자료로 직접 확인되지 않은 인과관계는 '~때문이다'라고 단정하지 말고 '배경 중 하나로 볼 수 있다', '함께 살펴볼 요소다'처럼 표현할 것
- 정비사업·교통·개발계획과 거래 증가가 같은 시기에 나타났더라도 직접적인 원인으로 확정하지 말 것
- 글 끝에는 매수 권유 대신 앞으로 확인할 체크포인트와 3줄 요약을 넣을 것
...
④ 최근 실거래에서 눈여겨볼 점
⑤ 입지와 생활권
⑥ 최신 교통·정비·공급 이슈 — 실제 관련 있고 확인된 내용만
⑦ 앞으로 체크할 것
⑧ 3줄 요약

[이미지 위치 — 본문 안에 반드시 표시]
아래 문구를 각각 한 줄로 단독 표시하고, 앞뒤로 한 줄씩 비워줘.
- 도입부가 끝난 뒤: [이미지 1 — 썸네일]
- 최근 6개월 가격 흐름 설명 직후: [이미지 2 — 최근 6개월 시세 그래프]
- 입지와 생활권 설명 직전 또는 직후: [이미지 3 — 입지 인포그래픽]

[네이버 블로그 붙여넣기용 띄우기 — 최우선 형식 규칙]
- 완전히 비어 있는 빈 줄을 절대 만들지 말 것
- 모든 문단 사이 간격용 줄은 ASCII 일반 스페이스(U+0020) 정확히 1개만 들어 있는 줄로 만들 것
- 탭, 여러 칸 공백, NBSP, 특수 공백 문자로 대체하지 말 것
- 반드시 '문장 → 엔터 → 스페이스 1칸만 있는 줄 → 엔터 → 다음 문장' 순서를 지킬 것
- 제목 다음에도 동일한 스페이스 1칸 간격 줄을 1개 넣을 것
- 일반 본문은 한 문장마다 동일한 간격 줄을 넣어 '한 문장 = 한 문단' 형태를 유지할 것
- 소제목 앞뒤에도 각각 동일한 스페이스 1칸 간격 줄을 넣을 것
- 이미지 위치 문구는 한 줄 단독으로 두고 위아래에 각각 동일한 간격 줄을 넣을 것
- 3줄 요약도 각 문장 사이에 동일한 간격 줄을 넣을 것
- 일반 텍스트가 있는 줄의 시작에는 공백을 넣지 말고, 간격용 줄에만 스페이스 1칸을 둘 것
- 출력 직전에 모든 문단 사이 줄을 검사해 '완전 빈 줄'이 남아 있으면 스페이스 1칸 줄로 바꾼 뒤 출력할 것
- 숫자와 단위는 붙여 쓰되, 문장 안의 단어 띄어쓰기는 자연스러운 표준 한국어로 작성할 것
- 모바일에서 텍스트가 한 덩어리로 보이지 않도록 2문장 이상을 한 문단에 붙이지 말 것
- 이미지 위치 문구도 본문 일부로 그대로 출력할 것

[최종 출력 형식 — 매우 중요]
아래 3가지만 출력해.
1) 최종 제목 1개 — 본문보다 크게 적용하기 쉬운 독립된 제목 줄
2) 최종 본문 — 일반 본문은 기본 크기, 소제목만 본문보다 크게 적용하기 쉬운 독립된 줄
3) 네이버 태그 7개

제목 후보, 작성 설명, 출처 목록, 검색 요약, 내부링크, 내부링크 추천, 기타 부가 설명은 모두 출력하지 마.
태그는 단지명·지역명·핵심 검색어 중심으로 중복 없이 정확히 7개만 작성해줘.

[출력 직전 최종 자가검수 — 화면에는 출력하지 말 것]
1. 최종 제목이 정확히 1개만 있는지 확인할 것
2. 네이버 태그가 중복 없이 정확히 7개인지 확인할 것
3. 제목·본문·소제목·이미지 위치·3줄 요약 사이에 완전히 비어 있는 줄이 남아 있지 않고, 간격용 줄마다 ASCII 스페이스 1칸만 들어 있는지 확인할 것
4. 제공되지 않은 실거래가·월 대표값·거래량·변화율 등 숫자를 임의로 만들지 않았는지 확인할 것
5. '최근 실거래가/개별 실거래가'와 '월 대표값'을 서로 혼동하거나 하나의 가격처럼 섞어 쓰지 않았는지 확인할 것
- 위 5개 중 하나라도 어기면 스스로 수정한 뒤 최종 결과만 출력할 것
