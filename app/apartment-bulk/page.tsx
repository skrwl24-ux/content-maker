...
[네이버 블로그 붙여넣기용 띄우기 — 이 형식을 기본값으로 적용]
- 완전히 비어 있는 빈 줄을 사용하지 말 것
- 문단 사이 간격용 줄에는 스페이스바 공백 1칸을 반드시 넣을 것
- 기본 형식은 '문장 → 엔터 → 스페이스바 1칸이 있는 줄 → 엔터 → 다음 문장'으로 할 것
- 제목 다음에도 스페이스바 1칸이 들어간 간격용 줄을 1개 둔 뒤 본문을 시작할 것
- 일반 본문은 한 문장마다 위 형식으로 떨어뜨려 '한 문장 = 한 문단'처럼 보이게 작성할 것
- 소제목 앞뒤에도 각각 스페이스바 1칸이 들어간 간격용 줄을 1개 넣을 것
- 소제목 바로 다음 첫 문장과 이후 문장 사이에도 같은 간격 방식을 유지할 것
- 이미지 위치 문구는 한 줄 단독으로 두고 위아래 각각 스페이스바 1칸이 들어간 간격용 줄을 1개 넣을 것
- 3줄 요약은 각 문장을 한 줄씩 따로 쓰고 문장 사이에 스페이스바 1칸이 들어간 간격용 줄을 1개 넣을 것
- 일반 텍스트가 있는 줄의 시작에는 불필요한 스페이스바 공백을 넣지 말 것
- 단, 문단 간격을 유지하기 위한 빈 줄에는 예외적으로 스페이스바 1칸을 반드시 넣을 것
