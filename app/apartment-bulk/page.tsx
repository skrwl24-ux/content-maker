...
- 가격상승형 제목 패턴은 '6개월 새 00억 올랐다', 가격하락형은 '6개월 새 00억 내렸다' 같은 구조를 기본 참고형으로 사용할 것.
- 여기서 '00억'은 제목 패턴을 설명하기 위한 자리표시자이며 최종 제목에는 그대로 출력하지 말 것.
- 최종 제목에서는 '00억' 자리에 해당 단지의 실제 대표값 변화액을 넣을 것.
- 예를 들어 변화액이 1.15억이면 '6개월 새 1.15억 올랐다'처럼 쓰고, 임의로 '1억대'라고 뭉뚱그리지 말 것.
...
[폰트·출력 서식 — 최우선]
- 최종 발행본 전체는 공백과 줄바꿈이 보존되는 문서형 작성 영역(writing block)을 1개만 사용해 출력할 것.
- writing block 밖에는 설명, 안내, 출처, 코멘트를 한 줄도 쓰지 말 것.
- writing block 내부 첫 줄에 최종 제목을 넣고, 그 아래 본문과 마지막 태그까지 모두 같은 블록 안에 넣을 것.
- 네이버 블로그 편집 기준은 제목 20pt / 소제목 18pt / 일반 본문 15pt / 태그 13~14pt로 잡을 것.
- 실제 최종 글에 '20pt', '18pt', '15pt', '13~14pt' 같은 편집 지시문은 출력하지 말 것.
- 제목은 가장 큰 제목 계층으로, 소제목은 제목보다 작고 본문보다 큰 소제목 계층으로 표현할 것.
- 일반 본문은 기본 본문 크기로 유지하고, 소제목은 굵게 표현할 것.
- 코드블록과 HTML은 사용하지 말 것.
- 문단 사이 간격은 '완전 빈 줄'이 아니라 ASCII 일반 스페이스(U+0020) 정확히 1개만 들어 있는 줄로 만들 것.
- 반드시 문장 → 엔터 → 스페이스바 1칸만 있는 줄 → 엔터 → 다음 문장 순서를 지킬 것.
- 제목 다음, 소제목 앞뒤, 이미지 문구 앞뒤, 모든 본문 문장 사이, 3줄 요약 문장 사이에 동일한 스페이스 1칸 줄을 넣을 것.
- 일반 텍스트가 있는 줄의 시작에는 공백을 넣지 말 것.
- 간격 줄에는 탭, 여러 칸 공백, NBSP, 전각 공백, 특수 공백을 사용하지 말고 일반 스페이스바 1칸만 사용할 것.
- 출력 직전 writing block 내부의 모든 문단 사이를 다시 검사하고, 완전히 비어 있는 줄이 있으면 스페이스바 1칸이 들어 있는 줄로 바꿀 것.
- 사용자가 writing block의 전체 내용을 복사해서 네이버 블로그에 붙여넣었을 때 문장 사이 한 줄 간격이 유지되는 형태를 최우선으로 할 것.
