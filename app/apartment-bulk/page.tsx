...
- 일반 본문 문장과 다음 문장 사이에는 빈 줄 1개가 보이도록 한 줄을 띄울 것
- PC와 모바일 모두 답답하지 않도록 짧은 문장을 우선하고 긴 문단을 만들지 말 것
- 소제목은 짧고 명확하게 작성할 것
- 네이버 블로그 편집 기준 베이스 폰트: 일반 본문 15pt
- 제목 20pt / 소제목 18pt / 일반 본문 15pt / 태그 13~14pt 기준으로 작성할 것
- 제목은 가장 크게, 소제목은 본문보다 확실히 크게 보이도록 구조를 구분할 것
...
- 실제 최종 출력에는 '20pt', '18pt', '15pt' 같은 편집 지시문을 본문 내용으로 적지 말 것
...
[베이스 폰트·띄우기 — 이 형식을 기본값으로 적용]
- 제목 다음에는 빈 줄 1개를 둔 뒤 본문을 시작할 것
- 일반 본문은 한 문장마다 엔터 2회로 빈 줄 1개를 넣어 문장을 서로 떨어뜨릴 것
- 즉, '한 문장 = 한 문단'처럼 보이게 작성할 것
- 소제목 앞에는 빈 줄 1개, 소제목 뒤에도 빈 줄 1개를 둘 것
- 소제목 바로 다음 첫 문장과 이후 문장들도 모두 동일하게 빈 줄 1개 간격을 유지할 것
- 이미지 위치 문구는 한 줄 단독으로 두고 위아래 각각 빈 줄 1개를 둘 것
- 3줄 요약은 각 요약 문장을 한 줄씩 따로 쓰고 문장 사이 빈 줄 1개를 둘 것
- 줄 시작에 불필요한 스페이스바 공백은 넣지 말 것
...
[최종 출력 형식 — 매우 중요]
아래 3가지만 출력해.
1) 최종 제목 1개 — 네이버 편집 시 20pt 기준
2) 최종 본문 — 소제목 18pt / 일반 본문 15pt 기준
3) 네이버 태그 7개 — 13~14pt 기준
