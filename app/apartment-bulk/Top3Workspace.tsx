...
import { Top3Work, IMAGE_SLOTS, bodyRequest, imageRequest, revision, missingEvidence, exportIssues } from "./top3-model";
...
export default function Top3Workspace({ workId, topic, materials, body, onBodyChange, data, onChange }: Props) {
  ...
  function promptButtons(text: string) {
    return <div className={styles.actions}>
      <a href={"https://chatgpt.com/?q=" + encodeURIComponent(text)} target="_blank" rel="noopener noreferrer">ChatGPT 열기</a>
  ...
  return <div className={styles.workspace}>
    <section className={styles.panel}>
      <h3>1. TOP3 자료 준비</h3>
      <p>직접 확인한 비교 자료를 입력하세요. 이 화면은 순위를 자동 계산하지 않습니다.</p>
    ...
    <section className={styles.panel}>
      <h3>2. 본문 요청서 · 작성 결과</h3>
      <p>자료가 부족하면 먼저 보충을 요청합니다. 지역 전체 순위를 확인하지 못했다면 제목과 본문에 분석 범위를 밝혀주세요.</p>
      {promptButtons(request)}
      <details><summary>본문 요청서 펼치기</summary><pre>{request}</pre></details>
